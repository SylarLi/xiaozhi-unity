...
public static class Builder
{
    ...
    public static void Build(BuildPresets buildPresets, BuildTarget buildTarget)
    {
    ...
    private static void CreateWindowsBuildZip(string exePath)
    {
        ...
        using (var archive = ZipFile.Open(zipPath, ZipArchiveMode.Create))
        {
            ...
            for (var index = 0; index < files.Length; index++)
            {
                ...
                if (!relativePath.StartsWith(backupFolderName) && !relativePath.StartsWith(burstDebugInfoFolderName))
