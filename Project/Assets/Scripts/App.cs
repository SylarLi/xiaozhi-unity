...
public class App : IDisposable
{
    ...
    private void InputAudio()
    {
        ...
        {
            if (_deviceState != DeviceState.Listening && _wakeWordDetect.IsDetectionRunning) _wakeWordDetect.Feed(data);
        }
    ...
    private void SetDecodeSampleRate(int sampleRate)
    {
        ...
        _outputResampler.Configure(codec.InputSampleRate, _opusDecodeSampleRate);
    }
    ...
    private void ConfigureAudioProcessing()
    {
        ...
        _audioProcessor.OnOutputData += data =>
        {
            Schedule(() =>
            {
                _opusEncoder.Encode(data.Span, opus => { Schedule(async () => { await _protocol.SendAudio(opus); }); });
                return Task.CompletedTask;
            }, AppTaskType.Background);
        };
    ...
    private void InitializeAudio()
    {
        ...
        if (codec.InputSampleRate != resampleRate)
        {
